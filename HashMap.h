...
// template definition
template<typename K, typename V, size_t tableSize, typename F = LongHashKey<tableSize> >
/**
...
public:
    ...
    /**
     * @return a vector<HashNode<K, V> *> which is all the nodes in the map
     */
    std::vector<HashNode<K, V> *> getAll() {
        std::vector<HashNode<K, V> *> nodes;
        // iterate through all the nodes and add them
        for (size_t i = 0; i < tableSize; ++i) {
            HashNode<K, V> *entry = table[i];
            while (entry != NULL) {
                nodes.push_back((new HashNode<K, V>(entry->getKey(), entry->getValue())));
                entry = entry->getNext();
            }
        }
        return nodes;
